...
// Define the type for patient input, excluding the 'id' field
...
export default function AddPatientForm() {
  ...
  // Handle changes in the patient form fields
  ...
  // Add a new contact method to the patient
  const addContactMethod = () => {
    if (newContact.value) {
      setPatient({
        ...patient,
        contactMethods: [...patient.contactMethods, newContact],
      });
      setNewContact({ contactMethod: "EMAIL", value: "" });
    }
  };

  // Remove a contact method from the patient
  const removeContactMethod = (index: number) => {
    const updatedContactMethods = [...patient.contactMethods];
    updatedContactMethods.splice(index, 1);
    setPatient({ ...patient, contactMethods: updatedContactMethods });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle errors, including duplicate patient ID
        const errorData = await response.json();
        if (errorData.code === "P2002") {
          toast({
            title: "Error",
            description: "A patient with this ID already exists",
            variant: "destructive",
          });
        } else {
      ...
      <Card className="w-full max-w-3xl mx-auto mt-8 shadow-lg rounded-lg overflow-hidden bg-white border border-gray-200">
        <CardHeader className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6">
          <CardTitle className="text-2xl font-semibold flex items-center">
            <User className="mr-3 h-6 w-6" /> Enter Patient Details
          </CardTitle>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-1 sm:gap-4">
                <div>
                  <Label
                    htmlFor="addressCity"
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-2 flex-wrap sm:flex-col sm:space-x-0 sm:space-y-2">
                <Select
                  onValueChange={(value) =>
